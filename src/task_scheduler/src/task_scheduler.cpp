...
public:
    ...
    TaskScheduler() {
        ...
        ROS_INFO("📍 走廊位置: (%.3f, %.3f)", LOBBY_X, LOBBY_Y);
        ROS_INFO("📍 出发区位置: (%.3f, %.3f)", START_X, START_Y);
    }
    ...
    void sendLobbyGoal() {
        ...
        ROS_INFO("📤 目标详情:");
        ROS_INFO("   frame_id: %s", goal.header.frame_id.c_str());
        ROS_INFO("   position: (%.3f, %.3f, %.3f)", 
                goal.pose.position.x, goal.pose.position.y, goal.pose.position.z);
        
        goal_pub_.publish(goal);
        ROS_INFO("📤 走廊目标已发布到 /nav_goal (时间戳: %.3f)", 
     ...
     */
    void returnToStart() {
        ROS_INFO("🏠 返回出发区 (%.3f, %.3f)", START_X, START_Y);
    ...
    void start() {
        ROS_INFO("▶️  开始执行任务调度");
        ROS_INFO("📍 走廊目标: (%.3f, %.3f)", LOBBY_X, LOBBY_Y);
        ROS_INFO("📍 出发区: (%.3f, %.3f)", START_X, START_Y);
        ros::Duration(1.0).sleep();
        
        // ===== 检查导航话题订阅者 =====
        ros::Duration(0.5).sleep();
        int subs = goal_pub_.getNumSubscribers();
        if (subs == 0) {
            ROS_WARN("⚠️⚠️⚠️ 警告: 没有节点订阅 /nav_goal 话题！");
            ROS_WARN("⚠️ 请确保导航节点 (move_base 或类似节点) 已启动");
            ROS_WARN("⚠️ 将等待 5 秒后继续...");
            ros::Duration(5.0).sleep();
        } else {
            ROS_INFO("✅ 检测到 %d 个订阅者连接到 /nav_goal", subs);
        }
        ...
        int retry_count = 0;  // ✅ 添加 retry_count 变量
        ...
        while (ros::ok() && !all_done_) {
            ...
            switch (current_state_) {
                case State::GOING_TO_LOBBY:
                    timeout_counter++;
                    if (timeout_counter % (5 * LOOP_RATE_HZ) == 0) {
                        ROS_INFO("⏳ 等待导航到走廊... 已等待 %d 秒", 
                                timeout_counter / LOOP_RATE_HZ);
                        ROS_INFO("📊 订阅者数量: %d", goal_pub_.getNumSubscribers());
                    }
                    ...
                    if (timeout_counter > NAV_TIMEOUT_SECONDS * LOOP_RATE_HZ) {
                        ...
                        if (retry_count <= 3) {
                            ROS_WARN("🔄 第 %d 次重试，重新发送导航目标...", retry_count);
                            sendLobbyGoal();
                        } else {
                            ROS_WARN("⚠️ 多次重试失败，跳过走廊直接进入等待人脸识别");
                            retry_count = 0;
                            transitionTo(State::WAITING_FACE);
                            ROS_INFO("👤 等待人脸识别唤醒...");
                        }
